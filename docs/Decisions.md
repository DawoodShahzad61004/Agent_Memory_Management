...
## ADR-031 · Add `ENABLE_PRUNING` and `MIN_PRUNE_BUDGET` as explicit pruning gates

| Field | Detail |
|---|---|
| **Decision** | Add two new `config.py` constants that gate `consolidate.py::rerank_and_prune()`: `ENABLE_PRUNING` (bool, default `True`) — a hard on/off switch — and `MIN_PRUNE_BUDGET` (int, default `2_000`) — the corpus's total `len(memory.content)` across all memories must clear this character budget before anything is pruned, independent of `ENABLE_PRUNING`. |
| **Date** | 2026-09-04 |
| **Context** | `rerank_and_prune()`'s existing behavior — a corpus under 5 memories prunes nothing — was never a deliberate guard; it was a side effect of `floor(n * 0.20)` rounding to zero below `n=5`. That meant the actual protection was tied to memory *count*, not to how much content that count represented (5 very long memories were pruned same as 5 short ones would not have been), and there was no way to disable pruning outright for inspection or debugging runs. |
| **Options Considered** | Leave the `floor()` side effect as the only implicit guard (undocumented, breaks the moment a corpus crosses 5 memories regardless of size) · a boolean flag only (can fully disable pruning, but doesn't protect a small-but-numerous set of short entries) · a character-budget threshold only (protects small corpora by actual size, but can't fully disable pruning for a caller that wants it off) · both a boolean flag and a character-budget threshold. |
| **Chosen Solution** | Both — `ENABLE_PRUNING` and `MIN_PRUNE_BUDGET`, checked independently. |
| **Rationale** | A boolean alone can't distinguish "a handful of short memories, worth protecting" from "a handful of very long ones, still worth pruning." A size guard alone can't let a caller turn pruning off entirely. Character count (not memory count) is the right unit for the size guard, since the old `floor()`-based side effect's actual protective value was always about how much content existed, not how many records it was split across. |
| **Impact** | `mem_manage/consolidate.py::rerank_and_prune()` reads both live from `config` each call, matching the existing `PRUNE_BOTTOM_PERCENT` pattern. The old `floor()`-based behavior still exists mechanically (a corpus can still fall below the 1-pruned-item threshold by count) but is no longer the documented safety net — `MIN_PRUNE_BUDGET` is. New tests in `test_consolidate.py::TestPruneGating` and `test_config.py`; existing prune-mechanics tests needed `MIN_PRUNE_BUDGET` monkeypatched, since their fixtures are shorter than 2000 chars. 113/113 tests passed. |

---

## ADR-032 · Complete-linkage grouping for near-duplicate merging, not anchor-only or transitive closure

| Field | Detail |
|---|---|
| **Decision** | `find_near_duplicate_groups()` groups candidates via **complete-linkage**: a candidate joins a group only if it clears `MERGE_SIMILARITY_THRESHOLD` against *every* member already claimed in that group, not just the group's anchor (first claimed member). |
| **Date** | 2026-09-04 |
| **Context** | The prior implementation compared each unclaimed candidate only against the group's anchor — not transitive closure, but also not requiring agreement with every member. This produced incomplete or wrong clusters: a genuine multi-entry topic chain could split into a pair plus an orphaned singleton if the chain's weakest link happened to fall on a non-anchor pair, and — more seriously — a candidate could be pulled into a group purely because it resembled the anchor specifically, even if it had nothing to do with the anchor's actual topic (the mechanism behind BUG-011's false merge). |
| **Options Considered** | (1) Union-find / connected components — any similarity edge above threshold chains entries together transitively; cheapest change, but classic single-linkage drift, where one bridging pair can pull two otherwise-unrelated entries into the same group. (2) Complete-linkage growth — a candidate must be similar to every current group member, not just the anchor; still a small diff to the existing loop, no drift risk. (3) Centroid/average-linkage — maintain a running mean embedding per group, candidate joins if similar enough to the centroid; a middle ground, but the group's "prototype" drifts as members are added, so a later candidate can pass even though it's genuinely far from one real member. (4) Library clustering (`sklearn.AgglomerativeClustering` with `linkage='complete'`, or HDBSCAN) — `AgglomerativeClustering` with complete linkage is functionally identical to option 2 but pulls in a new dependency to re-implement ~10 lines already hand-rollable; HDBSCAN solves a variable-cluster-density problem this dataset doesn't have, and its `min_cluster_size` knob drops the similarity-threshold semantics already tuned (`MERGE_SIMILARITY_THRESHOLD = 0.60`). |
| **Chosen Solution** | (2) Complete-linkage growth. |
| **Rationale** | The merge step concatenates and LLM-summarizes an entire group into one memory, so a **false merge is costlier than a missed merge** here — a wrongly-folded-together memory can't be un-merged, while a borderline entry left as its own singleton is still individually stored and retrievable. Complete-linkage is the smallest-diff option that eliminates single-linkage chaining/drift risk without adding a dependency. This was tested directly against a real 5-entry narrative (`learnings.md` records 0-4, about a Groq migration): complete-linkage correctly leaves one entry (a reviewer complaint) out of the merge, because it's genuinely below-threshold against one real group member (0.447) even though it clears threshold against another (0.61-0.64) — confirmed, on follow-up, to be the intended trade-off of this rule and not a defect. |
| **Impact** | `mem_manage/services/dedup_merge.py::find_near_duplicate_groups()`. Added `test_complete_linkage_rejects_a_candidate_not_similar_to_every_member` (a "star" case: a candidate similar to the anchor but not to a second group member — the old anchor-only algorithm would have wrongly merged all three; complete-linkage correctly excludes the candidate). 114/114 tests passed at the time this landed. |
